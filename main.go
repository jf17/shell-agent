...
ВНИМАТЕЛЬНО читай запрос и выполняй ВСЕ указанные инструкции, включая переводы и преобразования.
...
- "создай файл с именем Красный и переведи на английский" -> touch Red
- "создай три файла: Красный, Желтый, Зеленый с расширением .txt, переведи имена на английский" -> touch Red.txt Yellow.txt Green.txt

ВАЖНО: Если в запросе есть инструкция перевести что-то на другой язык, обязательно выполни перевод!
Переводы: Красный=Red, Желтый=Yellow, Зеленый=Green, Синий=Blue, Черный=Black, Белый=White
...
// validateCommand проверяет, соответствует ли команда требованиям запроса
func (sa *ShellAgent) validateCommand(query, command string) bool {
	queryLower := strings.ToLower(query)
	commandLower := strings.ToLower(command)
	
	// Проверяем переводы
	if strings.Contains(queryLower, "переведи") || strings.Contains(queryLower, "на английский") {
		// Если просят перевести, но в команде остались русские слова
		russianWords := []string{"красный", "желтый", "зеленый", "синий", "черный", "белый"}
...
func (sa *ShellAgent) intelligentShellAgent(query string) string {
	...
	// Проверяем качество команды
	if !sa.validateCommand(query, command) {
		fmt.Printf("⚠️ Команда может не соответствовать всем требованиям запроса\n")
		fmt.Printf("🔄 Попытка улучшить команду...\n")
		
		// Повторный запрос с более детальным промптом
		improvedCommand, err := sa.naturalLanguageToCommand(query + " (ОБЯЗАТЕЛЬНО выполни ВСЕ инструкции из запроса!)")
		if err == nil && sa.validateCommand(query, improvedCommand) {
			command = improvedCommand
			fmt.Printf("✅ Команда улучшена\n")
		}
	}
